...
const YourFormComponent = () => {
  ...
  const sendDataToServer = async () => {
    try {
      const response = await fetch('http://localhost:5173/api/highriseSND', {
        method: 'POST',
        headers: {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <label>
        Plants:
        <input type="number" name="plants" value={formData.plants} onChange={handleChange} className='formInput' />
      </label>

      <label>
        Defuse:
        <input type="number" name="defuse" value={formData.defuse} onChange={handleChange} className='formInput' />
      </label>
      <br />
